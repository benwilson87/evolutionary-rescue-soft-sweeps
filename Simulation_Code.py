#Author: Ben Wilson <bawilson@stanford.edu>
#PI: Dmitri Petrov
...
def main(parameter_list):
	...
	#Parameters to be set using a parameter list
	...
	number_simulations = 1 #Number of simulations you want to do for each parameter set

	#Example for how to create simulation
	while(number_simulations):
		numpy.random.seed() #Reseed random number generator if running in parallel
		...
		#Output lineages to a file with identifying information
		...
		number_simulations-=1 #Decrease counter for simulations after each simulation has completed
	return 0



#----------Run main method---------
#Example of how you might run in parallel by creating lists of parameters to explore
K_max_list = [110000]
b_wildtype_list = [0.99,0.97,0.9,0.7,0.0]
d_wildtype_list = [1.0]
b_mutant_list = [1.1,1.3,2.0]
d_mutant_list = [1.0]
mu_list = [0.00001,0.0001,0.001]
w_0_list = [10**4]
start = time.time()

number_of_parallel_processes = 20 #Number of independent processes you want to spawn

if __name__ == '__main__':
	#main()
	pool = Pool(processes=number_of_parallel_processes) #multiprocessing pool
	parameter_space = itertools.product(K_max_list,b_wildtype_list,d_wildtype_list,b_mutant_list,d_mutant_list,mu_list,w_0_list) #Take Cartesian product for parameter space
	results = pool.map(main, parameter_space)
